...
def pdlp_algorithm(K, m_ineq, c, q, l, u, device, max_iter=100_000, tol=1e-4, verbose=True, restart_period=40, precondition=False, primal_update=False, adaptive=False, data_precond=None):
    ...
    x_prev = x.clone()
    y_prev = y.clone()
    x_normalized_prev = None
    y_normalized_prev = None
    x_avg_normalized_prev = None
    y_avg_normalized_prev = None
    ...
    while k < max_iter:
        ...
        while k < max_iter:
            ...
            x_normalized = x / k
            y_normalized = y / k
            x_avg_normalized = 2 * x_sum / (k * (k + 1))
            y_avg_normalized = 2 * y_sum / (k * (k + 1))

            if k >= 2 and x_normalized_prev is not None and y_normalized_prev is not None:
                diff_x_norm = torch.norm(x_normalized - x_normalized_prev)
                diff_y_norm = torch.norm(y_normalized - y_normalized_prev)
                
                if diff_x_norm < 1e-4 and torch.norm(x_normalized) > 1e-3:
                    certificate_flag = "DUAL_INFEASIBLE (Normalized Iterate)"
                    certificate_iter = k
                    if verbose:
                        print(f"[PDLP] Dual infeasibility (normalized x convergence) detected at iter {k}")
                    break
                
                if diff_y_norm < 1e-4 and torch.norm(y_normalized) > 1e-3:
                    certificate_flag = "PRIMAL_INFEASIBLE (Normalized Iterate)"
                    certificate_iter = k
                    if verbose:
                        print(f"[PDLP] Primal infeasibility (normalized y convergence) detected at iter {k}")
                    break
            
            if k >= 2 and x_avg_normalized_prev is not None and y_avg_normalized_prev is not None:
                diff_x_avg = torch.norm(x_avg_normalized - x_avg_normalized_prev)
                diff_y_avg = torch.norm(y_avg_normalized - y_avg_normalized_prev)
                
                if diff_x_avg < 1e-4 and torch.norm(x_avg_normalized) > 1e-3:
                    certificate_flag = "DUAL_INFEASIBLE (Normalized Average)"
                    certificate_iter = k
                    if verbose:
                        print(f"[PDLP] Dual infeasibility (normalized average x convergence) detected at iter {k}")
                    break
                
                if diff_y_avg < 1e-4 and torch.norm(y_avg_normalized) > 1e-3:
                    certificate_flag = "PRIMAL_INFEASIBLE (Normalized Average)"
                    certificate_iter = k
                    if verbose:
                        print(f"[PDLP] Primal infeasibility (normalized average y convergence) detected at iter {k}")
                    break
            
            x_prev.copy_(x)
            y_prev.copy_(y)
            if k >= 1:
                x_normalized_prev = x_normalized.clone()
                y_normalized_prev = y_normalized.clone()
                x_avg_normalized_prev = x_avg_normalized.clone()
                y_avg_normalized_prev = y_avg_normalized.clone()
                    ...
                    if (m_ineq == 0 or torch.all(Gdx >= -tol_feas)) and (c.T @ dx < tol_obj):
                        ...
                        for i in range(x.shape[0]):
                            ...
                            if not (
                                ...
                                (u_i == float('inf') and dx_i >= -tol_feas) or
                                (l_i == -float('inf')  and dx_i <= tol_feas)
                            ):
                                bounds_ok = False
                                break
                        if bounds_ok:
...
class Args:
    device = 'gpu'
    instance_path = '/content/PDLP-AMD-RIPS/datasets/Netlib/infeasible'
    tolerance = 1e-4
    ...
    for mps_file in mps_files:
        ...
        try:
            with Timer("Solve time") as t:
                ...
                x, prim_obj, k, n, j = pdlp_algorithm(K, m_ineq, c, q, l, u, device, max_iter=1000000, tol=tol, verbose=True, restart_period=40, precondition=precondition,primal_update=primal_weight_update, adaptive=adaptive_stepsize, data_precond=dt_precond)
