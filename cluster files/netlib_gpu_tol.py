...
#             # Normalize for termination check
#             primal_tol = primal_residual / (1 + q_norm)
#             dual_tol = dual_residual / (1 + c_norm)
#             gap_tol = duality_gap / (1 + abs(prim_obj) + abs(adjusted_dual))

#             if verbose:
#                 print(f"[{k}] Prim Obj: {prim_obj:.4f}, Dual Obj: {adjusted_dual:.4f}, Gap: {duality_gap:.2e}, "
#                     f"Prim Tol: {primal_tol:.2e}, Dual Tol: {dual_tol:.2e}, Gap Tol: {gap_tol:.2e}")

#             if (primal_tol <= tol and dual_tol <= tol and gap_tol <= tol):
#                 if verbose:
#                     print(f"Converged at iteration {k}")
#                 break
        
#         if (perf_counter() - start_time) >= 35:
#             if verbose:
#                 print(f"Terminated due to time limit at iteration {k}")
#             break
          
#     return x, prim_obj.cpu().numpy(), k, primal_tol, dual_tol, gap_tol

def pdhg_torch(c, G, h, A, b, l, u, is_neg_inf, is_pos_inf, l_dual, u_dual, device, verbose=True, term_period=1000):
    n = c.shape[0]
    m_ineq = G.shape[0] if G.numel() > 0 else 0
    m_eq = A.shape[0] if A.numel() > 0 else 0

    K = torch.vstack([mat for mat in [G, A] if mat.numel() > 0]).to(device)
    q = torch.vstack([vec for vec in [h, b] if vec.numel() > 0]).to(device)
    c = c.to(device)
    ...
    tol_levels = [1e-2, 1e-4, 1e-8]
    tol_hits = {
        'primal': {tol: None for tol in tol_levels},
        'dual': {tol: None for tol in tol_levels},
        'gap': {tol: None for tol in tol_levels},
    }
    ...
    while True:
        ...
        if k % term_period == 0:
            ...
            for tol in tol_levels:
                if primal_tol <= tol and tol_hits['primal'][tol] is None:
                    tol_hits['primal'][tol] = k
                if dual_tol <= tol and tol_hits['dual'][tol] is None:
                    tol_hits['dual'][tol] = k
                if gap_tol <= tol and tol_hits['gap'][tol] is None:
                    tol_hits['gap'][tol] = k

            if verbose:
                print(f"[{k}] Obj: {prim_obj:.4f}, Dual: {adjusted_dual:.4f}, "
                      f"Gap: {duality_gap:.2e}, PrimTol: {primal_tol:.2e}, "
                      f"DualTol: {dual_tol:.2e}, GapTol: {gap_tol:.2e}")

            # Stop if all three conditions reached 1e-8
            if (primal_tol <= 1e-8 and dual_tol <= 1e-8 and gap_tol <= 1e-8):
                break

        if (perf_counter() - start_time) >= 750:
            if verbose:
                print(f"Time limit hit at iter {k}")
            break

    return (
        x, prim_obj.cpu().item(), k,
        primal_tol, dual_tol, gap_tol,
        tol_hits
    )
...
if __name__ == '__main__':
       ...
       # --- Solve ---
        try:
            with Timer("Solve time") as t:
                x, obj, k, prim_tol_val, dual_tol_val, gap_tol_val, tol_hits = pdhg_torch(c, G, h, A, b, l, u, is_neg_inf, is_pos_inf, l_dual, u_dual, device, verbose=False, term_period=1000)

            time_elapsed = t.elapsed

            status = "Solved (all tol ≤ 1e-8)" if (prim_tol_val <= 1e-8 and dual_tol_val <= 1e-8 and gap_tol_val <= 1e-8) else "Terminated (time or tol)"
            ...
            results.append({
                ...
                'Iterations (k)': k,
                'Time (s)': time_elapsed,
                'Final Primal Tol': prim_tol_val,
                'Final Dual Tol': dual_tol_val,
                'Final Gap Tol': gap_tol_val,
                'Primal @ 1e-2': tol_hits['primal'][1e-2],
                'Primal @ 1e-4': tol_hits['primal'][1e-4],
                'Primal @ 1e-8': tol_hits['primal'][1e-8],
                'Dual @ 1e-2': tol_hits['dual'][1e-2],
                'Dual @ 1e-4': tol_hits['dual'][1e-4],
                'Dual @ 1e-8': tol_hits['dual'][1e-8],
                'Gap @ 1e-2': tol_hits['gap'][1e-2],
                'Gap @ 1e-4': tol_hits['gap'][1e-4],
                'Gap @ 1e-8': tol_hits['gap'][1e-8],
                'Status': status
            })

            print(f"Finished: {mps_file}, Time: {time_elapsed:.2f}s, Iter: {k}, Obj: {obj:.4f}, Status: {status}")

            #     x, obj, k, prim_tol_val, dual_tol_val, gap_tol_val = pdhg_torch(c, G, h, A, b, l, u, is_neg_inf, is_pos_inf,
            ...
            #     status = "Terminated (time limit reached)"
            # elif prim_tol_val <= tol and dual_tol_val <= tol and gap_tol_val <= tol:
            #     status = "Solved"
            # else:
            #     status = "Terminated (other)"
    ...
    df.to_excel('pdhg_results_gpu_tol.xlsx', index=False)
    print("\nAll done. Results saved to pdhg_results_gpu_tol.xlsx.")