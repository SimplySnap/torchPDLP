...
def pdhg_torch(c, G, h, A, b, l, u, is_neg_inf, is_pos_inf, l_dual, u_dual, device, max_iter=100000, tol=1e-2, verbose=True, term_period=1000):
    ...
    # # Initialize x with random values within its bounds [l, u]
    # # This assumes u is large enough; rand() gives values in [0, 1)
    # x = torch.rand((n, 1), device=device)
    # x = torch.clamp(x, min=l, max=u) # Ensure x starts within bounds

    # # Initialize y with random normal values to break symmetry
    # y = torch.randn((K.shape[0], 1), device=device)
    x_old = x.clone()
    y_old = y.clone()
    ...
    for k in range(1, max_iter+1):
        ...
        # --- CALCULATE NEW STEP SIZES ---
            ...
            print(eta,eta_bar,eta_prime_term2)
            if eta <= eta_bar:
                eta = eta_prime
                print("eta set",eta)
                break
...
if __name__ == '__main__':
    ...
    df.to_excel('pdhg_results_ass.xlsx', index=False)
    print("\nAll done. Results saved to pdhg_results_ass.xlsx.")