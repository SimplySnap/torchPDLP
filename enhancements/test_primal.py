...
def pdhg_torch(c, G, h, A, b, l, u, is_neg_inf, is_pos_inf, l_dual, u_dual, device, max_iter=100000, tol=1e-2, verbose=True, term_period=1000):
    ...
    for k in range(max_iter):
        ...
        if k % term_period == 0:
            ...
            omega = primal_weight_update(x_prev, x, y_prev, y, omega, 0.5)
            tau = eta / omega
            sigma = eta * omega
            x_prev = x
            y_prev = y
...
if __name__ == '__main__':
    ...
    df.to_excel('pdhg_results_primal.xlsx', index=False)
    print("\nAll done. Results saved to pdhg_results_primal.xlsx.")